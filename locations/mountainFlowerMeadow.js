...
function _fm_exitSign(ctx, period) {
...
function drawFlowerMeadowScene(ctx, { px, py, t, period, mtn, cw, ch }) {
  ...
  _fm_sky(ctx, W, 400, period);
  if (period === 'night') _fm_stars(ctx, t, W, 400);
  else if (period === 'day' || period === 'morning') _fm_sunbeams(ctx, t, W, 400);

  _fm_ground(ctx, W, 400, period);
  _fm_trees(ctx, W, 400, period, t);
  _fm_grass(ctx, t, W, 400, period);
  _fm_moss(ctx, W, 400, period);
  _fm_flowers(ctx, t, W, 400, period);
  _fm_stream(ctx, t, W, 400, period);
  _fm_smallStones(ctx, W, 400, period);
  _fm_petals(ctx, t, W, 400, period);

  const szState = mtn.subZoneState.flowerMeadow;
  _fm_butterflies(ctx, t, szState);
  _fm_pickups(ctx, t, szState, W, 400);
  if (period === 'night') _fm_fireflies(ctx, t, W, 400);
  _fm_exitSign(ctx, period);

  if (typeof drawCat === 'function') {
    drawCat(ctx, { x: px, y: py, t, facing: mtn.facing, moving: mtn.isMoving, food: 80, mood: 88 });
  }

  _fm_wind(ctx, t, W, 400, period);
  _fm_zoneLabel(ctx, W, 400, period);
