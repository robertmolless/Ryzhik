/* ===================================================
   ГОРНАЯ ТРОПА — mountains.js
   «Соня: Тропа в горы» — новая глава
   Загружается после interior.js, до game.js
   =================================================== */
'use strict';

/* ════════════════════════════════════════════════
   ПРЕДМЕТЫ
   Определены здесь; game.js мержит через:
   Object.assign(ITEMS, MOUNTAIN_ITEMS)
   ════════════════════════════════════════════════ */
...
/* ════════════════════════════════════════════════
   КВЕСТЫ
   Определены здесь; game.js мержит через:
   QUESTS.push(...MOUNTAIN_QUESTS)
   ════════════════════════════════════════════════ */
const MOUNTAIN_QUESTS = [
  {
    id:'q_sonya_mtn1', title:'Тропа в горы',  icon:'⛰️',
    desc:'Соня показывает горную тропу. Войди в горы и найди её у смотровой площадки.',
    steps:['Войди в горы','Найди Соню у площадки','Поговори с ней'],
    reward:{ xp:25, trust:'sonya' }, npc:'sonya', unlock:false,
  },
  {
    id:'q_sonya_mtn2', title:'Горный цветок', icon:'🌼',
    desc:'На тропе растут редкие цветы. Найди горный цветок у больших камней и отдай Соне.',
    steps:['Найди горный цветок','Отдай Соне'],
    reward:{ xp:30, trust:'sonya' }, npc:'sonya', unlock:false,
  },
  {
    id:'q_sonya_mtn3', title:'Вид сверху',    icon:'🏔️',
    desc:'Поднимись к верхней площадке, найди гладкий камень и посиди с Соней у края.',
    steps:['Найди гладкий камень','Поднимись к верхней площадке','Посиди с Соней'],
    reward:{ xp:40, trust:'sonya', event:'mountain_vista' }, npc:'sonya', unlock:false,
  },
];

/* ════════════════════════════════════════════════
   ОБЪЕКТЫ СЦЕНЫ (аналог FURNITURE в interior.js)
   ════════════════════════════════════════════════ */
const MOUNTAIN_OBJECTS = [
  // Выход (левый край сцены)
  { id:'mt_exit',      x:50,  y:205, w:60,  h:95,  type:'trail_sign',      label:'Вернуться к лесу 🌲',  action:'exit_mountains',  blocking:false },
  // Лента на шесте (предмет квеста)
  { id:'mt_ribbon',    x:150, y:150, w:30,  h:60,  type:'ribbon_pole',     label:'🎀 Лента с тропы',       action:'pickup',     item:'trailRibbon',    blocking:false },
  // Смотровая площадка (нижняя, где стоит Соня)
  { id:'mt_viewpoint', x:325, y:130, w:105, h:78,  type:'viewpoint',       label:'⛰️ Смотровая площадка', action:'examine',         blocking:false },
  // Лавочка (блокирует движение)
...
/* ════════════════════════════════════════════════
   MOUNTAINS MANAGER
   Паттерн: BarnManager / MilitaryOfficeManager
   ════════════════════════════════════════════════ */
class MountainsManager {
  ...
  nearSonya() {
    return Math.hypot(this.px - 378, this.py - 172) < 95;
  }
  ...
  save() {
    return {
      ...
      unlockedFlag: this.unlockedFlag,
    };
  }
...
/* ════════════════════════════════════════════════
   DRAW MOUNTAIN SCENE
   Вызывается из game._draw() когда mountains.active
   ════════════════════════════════════════════════ */
function drawMountainScene(ctx, { px, py, t, period, mtn, sonyaNPC, cw, ch }) {
  const ROOM_W = 600;
  // Камера следует за игроком; центрирование на больших экранах
  ...
  if (showSonya && typeof drawHumanNPC === 'function') {
    drawHumanNPC(ctx, {
      id: 'sonya', x: 378, y: 170, t,
      facing: -1, moving: false,
      trust: sonyaNPC ? sonyaNPC.trust : 1, emotion: 'happy',
    });
    ctx.save();
    const bw = 52;
...
function _mtn_sky(ctx, W, H, period) {
  ...
  const palettes = {
    morning: ['#5a7ab8', '#8ab2d4', '#cce0f0'],
    day:     ['#4468a8', '#78a8cc', '#b4d2e8'],
    evening: ['#32285a', '#885068', '#cc8865'],
    night:   ['#080818', '#121230', '#202045'],
  };
...
function _mtn_distant(ctx, W, H, period) {
  ...
  ctx.beginPath();
  ctx.moveTo(0, H*0.46);
  const pts = [[0.12,0.18],[0.25,0.08],[0.37,0.22],[0.51,0.05],[0.65,0.20],[0.77,0.10],[0.91,0.25],[1,0.18],[1,0.46]];
  pts.forEach(([rx,ry]) => ctx.lineTo(W*rx, H*ry));
...
function _mtn_clouds(ctx, t, W, H, period) {
  ...
  const clouds = [
    { bx: 0.0,  by: H*0.10, r: 36, spd: 8.5,  phase: 0      },
    { bx: W*0.38, by: H*0.07, r: 26, spd: 5.5,  phase: W*0.5 },
    { bx: W*0.68, by: H*0.14, r: 30, spd: 6.8,  phase: W*0.3 },
  ];
  clouds.forEach(c => {
...
function _mtn_fog(ctx, t, W, H, period) {
  ...
  [[0.10,0.53,0.22,0.05,0],[0.44,0.49,0.28,0.042,1.4],[0.74,0.55,0.19,0.05,3.0]].forEach(([bx,by,rx,ry,ph]) => {
    ...
    ctx.beginPath();
    ctx.ellipse(W*bx + ox, H*by, W*rx, H*ry, 0, 0, Math.PI*2);
    ctx.fill();
  });
  ctx.restore();
}
...
function _mtn_ground(ctx, W, H, period) {
  ...
  // Верхнее плато (где стоит смотровая площадка и Соня)
  ctx.fillStyle = plat;
  ctx.beginPath();
  ctx.moveTo(W*0.30, H*0.39);
  ctx.lineTo(W*0.32, H*0.56);
  ctx.lineTo(W*0.99, H*0.56);
  ctx.lineTo(W,      H*0.43);
  ...
  [[W*0.26,H*0.82],[W*0.34,H*0.68],[W*0.36,H*0.58],[W*0.48,H*0.47],[W*0.58,H*0.38],[W*0.70,H*0.31]].forEach(([x,y]) => {
    ctx.fillStyle = pb;
    ctx.beginPath(); ctx.ellipse(x, y, 4, 2.5, 0.3, 0, Math.PI*2); ctx.fill();
  });
}
...
function _mtn_pines(ctx, W, H, period) {
  ...
  const pines = [
    { x:W*0.03, y:H*0.56, h:H*0.36, w:H*0.13 },
    { x:W*0.09, y:H*0.53, h:H*0.41, w:H*0.14 },
    { x:W*0.16, y:H*0.57, h:H*0.33, w:H*0.12 },
    { x:W*0.87, y:H*0.50, h:H*0.39, w:H*0.12 },
    { x:W*0.93, y:H*0.55, h:H*0.33, w:H*0.11 },
  ];
  pines.forEach(p => {
    ctx.fillStyle = trunk;
    ctx.fillRect(p.x - 5, p.y - 10, 10, 18);
    for (let tier = 0; tier < 3; tier++) {
      const ty = p.y - p.h * (0.35 + tier * 0.24);
      const tw = p.w * (1 - tier * 0.2);
      ctx.fillStyle = tier === 0 ? leaf : edge;
      ctx.beginPath();
      ctx.moveTo(p.x, ty - p.h * 0.14);
      ctx.lineTo(p.x - tw*0.5, ty + p.h * 0.14);
      ctx.lineTo(p.x + tw*0.5, ty + p.h * 0.14);
      ctx.closePath(); ctx.fill();
    }
  });
...
function _mtn_poles(ctx, t, mtn, period) {
  ...
  if (!mtn.pickedItems.has('mt_ribbon')) {
    ...
    ctx.save();
    ctx.strokeStyle = '#e06080'; ctx.lineWidth = 3; ctx.lineCap = 'round';
    ctx.shadowColor = '#ff80a0'; ctx.shadowBlur = 9;
    ctx.beginPath();
    ctx.moveTo(p1x, p1top + 7);
    ctx.quadraticCurveTo(p1x + 15 + wave, p1top + 11, p1x + 22 + wave*0.7, p1top + 18);
    ctx.stroke();
    ctx.restore();
  }

  // Декоративный шест 2 вдоль тропы
  ...
  ctx.beginPath();
  ctx.moveTo(p2x, p2top + 5);
  ctx.quadraticCurveTo(p2x + 12 + wave2, p2top + 9, p2x + 18 + wave2*0.6, p2top + 15);
  ctx.stroke();
}
...
function _mtn_rocks(ctx, W, H, period) {
  ...
  const rocks = [
    { x:260, y:148, w:68, h:58 },
    { x:145, y:258, w:78, h:62 },
    { x:490, y:175, w:62, h:52 },
    { x:382, y:243, w:32, h:22 },
    { x:308, y:192, w:24, h:17 },
    { x:445, y:265, w:28, h:20 },
  ];
  rocks.forEach(r => {
    ctx.fillStyle = base;
    ctx.beginPath(); ctx.ellipse(r.x+r.w*0.5, r.y+r.h*0.5, r.w*0.5, r.h*0.5, 0, 0, Math.PI*2); ctx.fill();
    ctx.fillStyle = hi;
    ctx.beginPath(); ctx.ellipse(r.x+r.w*0.4, r.y+r.h*0.38, r.w*0.2, r.h*0.14, -0.4, 0, Math.PI*2); ctx.fill();
  });
  // Маленькие горные цветы (декор, не предмет)
  if (period !== 'night') {
    [[198,268],[352,258],[462,248]].forEach(([fx,fy]) => {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        ctx.beginPath(); ctx.arc(fx+Math.cos(a)*5, fy+Math.sin(a)*5, 2.5, 0, Math.PI*2); ctx.fill();
      }
    });
  }
}

/* ── Нижняя площадка + лавочка ── */
function _mtn_platform(ctx, period) {
  ...
  // Каменный постамент площадки
  ctx.fillStyle = pC;
  ctx.beginPath();
  ctx.moveTo(325, 208); ctx.lineTo(430, 208);
  ctx.lineTo(440, 198); ctx.lineTo(315, 198); ctx.closePath(); ctx.fill();
  ctx.fillStyle = eC;
  ctx.fillRect(325, 130, 105, 68);
  // Перила
  ctx.strokeStyle = rC; ctx.lineWidth = 2.5;
  ctx.beginPath(); ctx.moveTo(330, 134); ctx.lineTo(425, 134); ctx.stroke();
  for (let rx = 335; rx <= 420; rx += 18) {
    ctx.beginPath(); ctx.moveTo(rx, 134); ctx.lineTo(rx, 165); ctx.stroke();
  }
  ...
  // Верёвка-декор над лавочкой
  ctx.strokeStyle = period === 'night' ? '#303028' : '#a08060';
  ctx.lineWidth = 1.5; ctx.setLineDash([4,3]);
  ctx.beginPath(); ctx.moveTo(bx+14,by-8);
  ctx.bezierCurveTo(bx+28,by-18, bx+52,by-14, bx+bw-14,by-7);
...
function _mtn_upper(ctx, period) {
  ...
  ctx.fillStyle = pC;
  ctx.beginPath();
  ctx.moveTo(uvx, uvy+uvh); ctx.lineTo(uvx+uvw, uvy+uvh);
  ctx.lineTo(uvx+uvw+12, uvy+uvh-10); ctx.lineTo(uvx+12, uvy+uvh-10);
  ctx.closePath(); ctx.fill();
  ...
  ctx.lineWidth = 1;
  [[uvx+14,uvy+8,uvx+35,uvy+8],[uvx+44,uvy+5,uvx+70,uvy+5],
   [uvx+10,uvy+18,uvx+40,uvy+18],[uvx+50,uvy+20,uvx+uvw-8,uvy+20]].forEach(([x1,y1,x2,y2]) => {
    ctx.beginPath(); ctx.moveTo(x1,y1); ctx.lineTo(x2,y2); ctx.stroke();
  });
  // Перила
  ctx.strokeStyle = rC; ctx.lineWidth = 2.5;
  ctx.beginPath(); ctx.moveTo(uvx+5, uvy+4); ctx.lineTo(uvx+uvw-5, uvy+4); ctx.stroke();
  for (let rx = uvx+10; rx <= uvx+uvw-10; rx += 20) {
    ctx.beginPath(); ctx.moveTo(rx, uvy+4); ctx.lineTo(rx, uvy+uvh*0.44); ctx.stroke();
  }
  // Флажок на верхней площадке
  ...
  ctx.beginPath();
  ctx.moveTo(uvx+uvw-10, uvy-22); ctx.lineTo(uvx+uvw+5, uvy-15); ctx.lineTo(uvx+uvw-10, uvy-8);
  ctx.closePath(); ctx.fill();
}
...
function _mtn_campfire(ctx, t, period) {
  ...
  if (period !== 'day') {
    ...
    ctx.bezierCurveTo(cx+sm,cy-36,cx-sm,cy-50,cx+sm*0.5,cy-64);
    ctx.stroke();
  }
  ctx.restore();
}
...
function _mtn_pickups(ctx, t, mtn, period) {
  ...
  // Горный цветок (mt_flower) — центр x:440, y:271
  if (!mtn.pickedItems.has('mt_flower')) {
    ctx.save();
    ctx.shadowColor = '#ffe050'; ctx.shadowBlur = 12 + glow * 8;
    ctx.font = '22px serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    ctx.fillText('🌼', 440, 271);
    ctx.restore();
  }

  // Гладкий камень (mt_stone) — центр x:484, y:128
  if (!mtn.pickedItems.has('mt_stone')) {
    ctx.save();
    ctx.shadowColor = '#c0b0a0'; ctx.shadowBlur = 10 + glow * 6;
    ctx.font = '20px serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    ctx.fillText('🪨', 484, 128);
    ctx.restore();
  }
}
...
function _mtn_exit_sign(ctx, period) {
  ...
  ctx.fillStyle = bC;
  if (ctx.roundRect) ctx.roundRect(sx+2, sy+1, sw-4, 30, 5);
  else ctx.rect(sx+2, sy+1, sw-4, 30);
  ctx.fill();
